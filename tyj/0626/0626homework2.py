...
#     prod_info = raw_input("请输入商品价格，或输入Q进行结算")
#     global product_num1
#     product_num1 = product_num1+1
#     if product_num1 ==3:
#         return "Q"
#     return 100
# #  依据输入的商品信息，计算商品的结算清单以及总价
# def goods_money(goods_list):
#     goods_list=[
#         [1,100,95],
#         [2,200,190],
#         [3,300,285]
...
while True:
    try:
        user_tel = int(input('请输入手机号'))
        user_disc = member_disc(user_tel)
        break
    except:
        print ("输入有误，请输入正确的手机号")
...
def payment(goods_list,user_disc):
    ...
    for i in range(len(goods_list)):
        ...
        total = total + goods_list(i)*user_disc
...
def format_out_msg(list,total):
    out_msg = "商品ID\t原价\t折后价"
    for prod in list:
        out_msg= out_msg + "%s\t,%s\t,%s\n" %(prod[0],prod[1],prod[3])
    out_msg+= "--------------------\n"
    out_msg+= "总价=\t%s" %total
    return out_msg

out_put = format_out_msg()
